...
function Install-PythonEmbed([string]$Version, [string]$TargetDir, [string]$DownloadsDir, [string]$EmbedUrl, [string]$GetPipUrl) {
    ...
    if (-not (Test-Path $zip)) {
        Log "Embed package: $EmbedUrl"
        Download $EmbedUrl $zip
    } else {
        Log "Reusing embed package: $zip"
    }
    ...
    if ($pth) {
        ...
        foreach ($line in $content) {
            if ($line -match '^#\s*import site') { $newContent += 'import site' }
...
try {
    ...
    Log "Python method from manifest: $pyMethod"
        ...
        if ($pythonExe) {
            ...
            if ($pyMethod -eq "embed_first" -or $pyMethod -eq "embed") {
                try {
                    ...
                    Log "Embed install failed: $($_.Exception.Message)"
                }
            }
            ...
            if (-not $pythonExe -and $pyMethod -ne "embed") {
                ...
                try {
                    if (-not (Test-Path $pyInstaller)) {
                        ...
                        Log "Reusing full installer: $pyInstaller"
                    }
                    $null = Install-PythonFull -InstallerPath $pyInstaller -TargetDir $pythonDir
                    Start-Sleep -Seconds 2
                    $pythonExe = Find-PythonExe $pythonDir
                } catch {
                    Log "Full installer failed: $($_.Exception.Message)"
                }
            }

            if (-not $pythonExe -and $pyMethod -eq "full_first") {
                try {
                    $pythonExe = Install-PythonEmbed -Version $ver -TargetDir $pythonDir -DownloadsDir $downloads -EmbedUrl $embedUrl -GetPipUrl $getPipUrl
                } catch {
                    Log "Embed fallback failed: $($_.Exception.Message)"
                }
            }

            if ($pythonExe) {
                Log "Python OK: $pythonExe"
                try { Log ("Python version check: {0}" -f (& $pythonExe --version 2>&1)) } catch {}
            } else {
    ...
    if ($InstallChrome) {
        ...
        try {
            ...
            Log "Chrome online=$($stable.version); installed=$installed"

            $chromeExePath = Join-Path $chromeRoot "chrome-win64\chrome.exe"
            $needChrome = ($installed -ne [string]$stable.version) -or (-not (Test-Path $chromeExePath))

            if ($needChrome) {
                $zip = Join-Path $downloads "chrome-$($stable.version).zip"
                if (-not (Test-Path $zip)) { Download ([string]$download.url) $zip }
                else { Log "Reusing Chrome zip: $zip" }
            ...
            if (Test-Path $chromeExe) { Log "Chrome OK: $chromeExe" }
            else { Log "WARNING: chrome.exe missing"; $chromeExe = "" }
        } catch {
    ...
    if ($InstallControlCenter) {
        ...
        if (-not (Test-Path $pkg)) { Download $url $pkg }
        else { Log "Reusing Control Center package: $pkg" }
    ...
    Step "Refreshing local installer files"
    foreach ($name in @("START_PLATFORM_INSTALLER.ps1","INSTALLER_CORE.ps1","START_INSTALLER_GUI.cmd","INSTALL_AutomationPlatform.bat","INSTALL_AutomationPlatform.ps1")) {
