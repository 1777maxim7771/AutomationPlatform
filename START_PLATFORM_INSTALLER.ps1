...
public class GlowButton : Button {
    public Color BaseColor { get; set; } = Color.FromArgb(37,181,151);
    public Color HoverColor { get; set; } = Color.FromArgb(68,226,190);
    public Color PressColor { get; set; } = Color.FromArgb(25,133,113);
    public Color EdgeColor { get; set; } = Color.FromArgb(112,255,224);
    ...
    GraphicsPath Round(Rectangle r,int rad){
    ...
    protected override void OnPaint(PaintEventArgs e){
        ...
        using(var sp=Round(shadow,8)) using(var sb=new SolidBrush(Color.FromArgb(80,0,0,0))) e.Graphics.FillPath(sb,sp);
        ...
        using(var path=Round(body,8)){
            using(var br=new LinearGradientBrush(body,c1,c2,LinearGradientMode.Vertical)) e.Graphics.FillPath(br,path);
            using(var pen=new Pen(over?EdgeColor:Color.FromArgb(90,EdgeColor),over?2f:1f)) e.Graphics.DrawPath(pen,path);
        }
...
function Tail([string]$Path,[int]$Count=16){if(Test-Path $Path){return((Get-Content -Path $Path -Tail $Count -ErrorAction SilentlyContinue)-join "`r`n")};return "Waiting for live bootstrap log..."}
function Set-Card($Panel,$Dot,$Title,$Value,[string]$State){
    $Value.Text=$State
    if($State -match 'OK|healthy|SKIP|PREVERIFIED|0\.5\.0|3\.13\.14'){$Dot.BackColor=[Drawing.Color]::FromArgb(66,235,174);$Panel.BackColor=[Drawing.Color]::FromArgb(22,48,46)}
...
$form.Size=New-Object Drawing.Size(980,690)
...
$btnAdvanced=New-Object GlowButton;$btnAdvanced.Text='ADVANCED';$btnAdvanced.Size=New-Object Drawing.Size(124,38);$btnAdvanced.Location=New-Object Drawing.Point(694,141);$btnAdvanced.Anchor='Top,Right';$btnAdvanced.BaseColor=[Drawing.Color]::FromArgb(42,79,112);$btnAdvanced.HoverColor=[Drawing.Color]::FromArgb(65,119,166);$btnAdvanced.EdgeColor=[Drawing.Color]::FromArgb(100,184,245);$form.Controls.Add($btnAdvanced)
$btnRefresh=New-Object GlowButton;$btnRefresh.Text='RESCAN';$btnRefresh.Size=New-Object Drawing.Size(116,38);$btnRefresh.Location=New-Object Drawing.Point(826,141);$btnRefresh.Anchor='Top,Right';$btnRefresh.BaseColor=[Drawing.Color]::FromArgb(42,79,112);$btnRefresh.HoverColor=[Drawing.Color]::FromArgb(65,119,166);$btnRefresh.EdgeColor=[Drawing.Color]::FromArgb(100,184,245);$form.Controls.Add($btnRefresh)
...
foreach($p in @($pyCard[0],$chCard[0],$ccCard[0])){$p.Anchor='Top,Left'}
...
$btnLogs=New-Object GlowButton;$btnLogs.Text='OPEN LOGS';$btnLogs.Size=New-Object Drawing.Size(140,44);$btnLogs.Location=New-Object Drawing.Point(28,590);$btnLogs.Anchor='Bottom,Left';$btnLogs.BaseColor=[Drawing.Color]::FromArgb(43,74,101);$btnLogs.HoverColor=[Drawing.Color]::FromArgb(62,111,151);$btnLogs.EdgeColor=[Drawing.Color]::FromArgb(110,184,235);$form.Controls.Add($btnLogs)
$btnInstall=New-Object GlowButton;$btnInstall.Text='INSTALL / UPDATE / REPAIR';$btnInstall.Size=New-Object Drawing.Size(320,52);$btnInstall.Location=New-Object Drawing.Point(622,584);$btnInstall.Anchor='Bottom,Right';$btnInstall.Font=New-Object Drawing.Font('Segoe UI Semibold',10);$form.Controls.Add($btnInstall)
...
function Update-LocalCards {
    ...
    Set-Card $pyCard[0] $pyCard[1] $pyCard[2] $pyCard[3] $(if($s.PythonOk){"OK  $($s.Python)"}else{$s.Python})
    Set-Card $chCard[0] $chCard[1] $chCard[2] $chCard[3] $(if($s.ChromeOk){"OK  $($s.Chrome)"}else{$s.Chrome})
    Set-Card $ccCard[0] $ccCard[1] $ccCard[2] $ccCard[3] $(if($s.CCOk){"OK  v$($s.CC)"}else{$s.CC})
}
...
function Parse-Live([string]$Text){
    ...
    if($Text -match '\[PYTHON[^\]]*\].*\[(SKIP|OK|PREVERIFIED)\]|\[PYTHON\]\[(SKIP|OK|PREVERIFIED)\]') { Set-Card $pyCard[0] $pyCard[1] $pyCard[2] $pyCard[3] 'OK / SKIP' }
    if($Text -match '\[CHROME\]\[(SKIP|OK|PREVERIFIED)\]'){Set-Card $chCard[0] $chCard[1] $chCard[2] $chCard[3] 'OK / SKIP'}
    if($Text -match '\[CHROME\]\[(DEFER_UPDATE|UPDATE_FAILED_USING_EXISTING)\]'){Set-Card $chCard[0] $chCard[1] $chCard[2] $chCard[3] 'WARN / DEFER'}
    if($Text -match '\[CONTROL_CENTER\]\[(OK|SKIP)\]'){Set-Card $ccCard[0] $ccCard[1] $ccCard[2] $ccCard[3] 'OK / CURRENT'}
...
$timer.Add_Tick({
    $root=$txtRoot.Text.Trim().Trim([char]0x22,[char]0x27).TrimEnd('\','/')
    $live=Join-Path $root 'logs\latest_bootstrap.log'
    $text=Tail $live 18
...
$btnInstall.Add_Click({
    ...
    try{
        ...
        $psi=New-Object Diagnostics.ProcessStartInfo;$psi.FileName='powershell.exe';$psi.Arguments="-NoProfile -ExecutionPolicy Bypass -File `"$runner`" -Root `"$root`" -ManifestUrl `"$manifest`"";$psi.UseShellExecute=$false;$psi.CreateNoWindow=$true;$psi.WorkingDirectory=$root
        $script:Proc=New-Object Diagnostics.Process;$script:Proc.StartInfo=$psi;[void]$script:Proc.Start();$script:RunStart=Get-Date
...
$form.Add_Shown({Update-LocalCards;$txtRoot.Focus()})
$form.Add_FormClosing({if($script:Proc -and -not $script:Proc.HasExited){$ans=[Windows.Forms.MessageBox]::Show('An install/update operation is still running. Close the panel anyway?','AutomationPlatform',[Windows.Forms.MessageBoxButtons]::YesNo,[Windows.Forms.MessageBoxIcon]::Warning);if($ans -ne [Windows.Forms.DialogResult]::Yes){$_.Cancel=$true}}})
